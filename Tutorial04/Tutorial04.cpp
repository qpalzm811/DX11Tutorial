...
//#endif
    ...
    bd.ByteWidth = sizeof( SimpleVertex ) * 8;  //缓冲区大小（以字节为单位）
    ...
    bd.BindFlags = D3D11_BIND_INDEX_BUFFER; //将一个缓冲区作为索引缓冲区绑定到输入汇编程序阶段。
	bd.CPUAccessFlags = 0;
	...
	g_Projection = XMMatrixPerspectiveFovLH( XM_PIDIV2, //90度自上而下的视场角（以弧度
...
//--------------------------------------------------------------------------------------
// Clean up the objects we've created 清理我们创建的对象
//--------------------------------------------------------------------------------------
void CleanupDevice()
{
...
//--------------------------------------------------------------------------------------
// Called every time the application receives a message 每次收到消息的时候调用
//--------------------------------------------------------------------------------------
...
void Render()
{
	...
	g_pImmediateContext->UpdateSubresource(  , 0, NULL, &cb, 0, 0 );
    ...
    //
    // Present our back buffer to our front buffer 后置buffer展示给前置buffer
    //
